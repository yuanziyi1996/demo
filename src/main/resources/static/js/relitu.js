...
function updateTextInput1(val) {
    ...
    console.log(document.getElementById('updateTextInput1').value)
}
...
function getBoundary(){
    ...
    bdary.get(name, function(rs){       //获取行政区域
        ...
        for(var i = 0; i < count; i++){
            var ply = new BMap.Polygon(rs.boundaries[i], {strokeWeight: 4, strokeColor: "#0000FF"}); //建立多边形覆盖物
...
function localsearch() {
    ...
    var place=document.getElementById("localvalue").value;
    local.search(place);
...
heatmapOverlay.setDataSet({data:points,max:700});
//是否显示热力图
...
function f(){
    ...
    map.addOverlay(heatmapOverlay);
    heatmapOverlay.setDataSet({data:points,max:m1});
    heatmapOverlay.show();
}
